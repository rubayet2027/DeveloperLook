...
const navLinks = [
  ...
  { label: 'International', href: '/international', hasDropdown: true },
  { label: 'About', href: '/about', hasDropdown: true },
  { label: 'Work', href: '/work', badge: '25' },
  { label: 'Careers', href: '/careers' },
  { label: 'Blog & Resources', href: '/blog', hasDropdown: true },
...
const mobileLinks = [
  ...
  { label: 'International', href: '/international', hasChevron: true },
...
export default function Header() {
  ...
  return (
    <>
      {/* Announcement bar */}
      <div className="w-full bg-mint text-dark text-center py-2.5 px-4 text-sm font-semibold relative z-50">
        <a href="#" className="inline-flex items-center gap-1.5 hover:opacity-70 transition-opacity">
          🚨 <span className="font-bold">The Category Leaderboard</span> - Live Now
        </a>
      </div>
        ...
        <div className="mx-auto px-5 lg:px-10 max-w-[1440px]">
          <div className="flex items-center justify-between h-[60px] lg:h-[68px]">
            ...
            <Link href="/" className="flex-shrink-0">
              <svg className={`h-[17px] w-auto transition-colors duration-300 ${scrolled ? 'fill-dark' : 'fill-white'}`} xmlns="http://www.w3.org/2000/svg" viewBox="0 0 168 21">
            ...
            <nav className="hidden xl:flex items-center gap-0.5">
              {navLinks.map((link) => (
                <Link
                  key={link.href}
                  href={link.href}
                  className={`relative group flex items-center gap-1 px-3 py-2 text-[13px] font-medium transition-colors duration-200 ${
                ...
                >
                  ...
                  {link.badge && (
                    <span className="ml-0.5 w-[18px] h-[18px] rounded-full bg-mint text-dark text-[9px] font-bold flex items-center justify-center">{link.badge}</span>
                  )}
                </Link>
              ))}
            </nav>
            ...
            <button
              ...
              className={`xl:hidden flex flex-col items-center justify-center w-10 h-10 gap-[5px] rounded-full border transition-colors duration-200 ${
                scrolled || menuOpen ? 'border-dark/20' : 'border-white/25'
              }`}
              aria-label={menuOpen ? 'Close' : 'Menu'}
            >
              <span className={`block w-[17px] h-[1.5px] transition-all duration-300 ${scrolled || menuOpen ? 'bg-dark' : 'bg-white'} ${menuOpen ? 'rotate-45 translate-y-[6.5px]' : ''}`}/>
              <span className={`block w-[17px] h-[1.5px] transition-all duration-300 ${scrolled || menuOpen ? 'bg-dark' : 'bg-white'} ${menuOpen ? 'opacity-0' : ''}`}/>
              <span className={`block w-[17px] h-[1.5px] transition-all duration-300 ${scrolled || menuOpen ? 'bg-dark' : 'bg-white'} ${menuOpen ? '-rotate-45 -translate-y-[6.5px]' : ''}`}/>
            </button>
          </div>
        </div>
