...
export default function ServicesSection() {
  ...
  // GSAP infinite marquee for "Chasing Consumers"
  useGSAP(() => {
  ...
  return (
    <section className="bg-site-bg pt-16 lg:pt-24 pb-0 overflow-hidden">
      <div className="px-5 lg:px-10 max-w-[1440px] mx-auto">
        <motion.div
          className="flex flex-col sm:flex-row sm:items-end justify-between gap-4 mb-10 lg:mb-14"
        ...
        >
          ...
          >
            ...
            <span className="inline-block w-[38px] h-[38px] sm:w-[48px] sm:h-[48px] rounded-lg overflow-hidden align-middle">
              <img src="https://images.unsplash.com/photo-1522071820081-009f0129c71c?w=200&h=200&q=80&auto=format&fit=crop" alt="" className="w-full h-full object-cover" />
            </span>
            {' '}Services
          </h2>
          <motion.a
        ...
        <motion.div
          className="grid grid-cols-1 md:grid-cols-2 gap-x-12 lg:gap-x-20"
          ...
          {services.map((service) => (
            ...
            >
              <div className="flex items-center justify-between">
                <span className="text-dark font-bold tracking-tight group-hover:text-dark/60 transition-colors duration-200"
                  style={{ fontSize: 'clamp(1.3rem, 3vw, 2.2rem)' }}>
                  {service.name}
                </span>
                <svg className="w-5 h-5 text-dark/30 group-hover:text-dark group-hover:rotate-45 transition-all duration-300 flex-shrink-0"
      ...
      {/* "Chasing Consumers" GSAP marquee */}
        ...
        <div ref={marqueeRef} className="flex whitespace-nowrap">
          {[0, 1, 2].map((set) => (
            <div key={set} className="flex-shrink-0 flex items-center">
              <span className="mx-4 text-dark font-bold tracking-tight select-none flex items-center gap-4"
